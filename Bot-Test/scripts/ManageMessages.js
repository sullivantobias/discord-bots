        ...
        if (!message.author.bot) {
            list.blacklist.forEach(word => {
                if (message.toString().toLowerCase().includes(word)) {
                    // get log channel //
                    const log = this.bot.channels.find(channel => channel.name === "logs")
                    // warning message
                    let warning = `${message.author} | '${word}' n'est pas autorisé, Attention au language 👿`
                    // check lang (fr by default) //
                    if (lang === 'EN')
                        warning = `${message.author} | '${word}' is forbidden , pay attention to your language 👿`
